...
## 20. Pixifi ↔ Scheduler reconciliation — manual check, not a cron/Zap (built 2026-08-07)

Originally scoped as an automated monthly cron + Zap (Catch Hook → Gmail); changed direction after discussing with Adi — a silent background check risks going unnoticed for months if it breaks, and tying it to a Zap added infra to maintain purely for scheduling. Landed instead as a **manual button** (`PixifiCheckButton.tsx`) directly above "Send availability request" on the Availability tracker page, styled distinctly (`.btn-rose`, globals.css — a new pink/rose variant, deliberately not blue like the other buttons, per Adi's request) so it reads as its own distinct action, not just another send button.

`checkPixifiReconciliation(month)` (`availability-tracker/actions.ts`) fetches `PIXIFI_ICS_FEED_URL` directly (plain `fetch`, no Zap involved at all for this one — the original design's "needs one more Zap" is no longer true) and calls into `src/lib/pixifi.ts` (pure logic, no I/O, same split as `scheduling.ts`/`types.ts`):
- `parseIcsEvents()` — minimal hand-rolled ICS/VEVENT parser (unfolds RFC 5545 line-folding, pulls `DTSTART`/`SUMMARY` per event). No `ics`/`node-ical` dependency added — Pixifi's feed only ever needs date + title extracted, not full RFC 5545 fidelity (recurrence rules, timezones, etc. are never used).
- `reconcile()` — same-date + fuzzy substring name match (normalizes case/punctuation and strips the make-up-day suffix, same regex as the webhook's saved-school lookup in §15) between Pixifi's events and this month's `flattenJobDays()` output. Returns both directions: `pixifiOnly` (booked in Pixifi, missing from Scheduler) and `schedulerOnly` (in Scheduler, no matching Pixifi event — possibly canceled).

**Scoped to the tracker page's currently-selected month**, not "all upcoming" — ties the check to the specific month Adi's about to send availability requests for, which is the whole point of where the button sits. Results render in-app only (no email) — a list of mismatches, or a green "no mismatches" banner, or an amber "no feed configured" banner if `PIXIFI_ICS_FEED_URL` is unset. Never auto-adds or auto-deletes anything, same as the original design.

**The Send button's confirm dialog now also asks "Did you check Pixifi for this month yet?"** as a reminder line (`SendAvailabilityButton.tsx`) — a plain text reminder every time, not state that tracks whether the button was actually clicked; Adi's ask was for a nudge, not enforcement.
